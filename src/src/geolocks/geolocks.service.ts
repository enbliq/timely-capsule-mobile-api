...
export class GeolocksService {
  ...
  async update(id: string, updateGeolockDto: UpdateGeolockDto): Promise<Geolock> {
    const updatedGeolock = await this.geolockModel.findByIdAndUpdate(id, updateGeolockDto, { new: true }).exec()
    if (!updatedGeolock) {
      throw new NotFoundException(`Geolock with ID ${id} not found`)
    }
